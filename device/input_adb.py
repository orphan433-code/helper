...
from device.adb import keyevent, swipe_raw, tap_raw, wake_screen
from core.config import bank_settings


def _jitter(x: float, y: float) -> tuple[int, int]:
    from bank.human import tap_xy
...
def dismiss_keyboard(*, interval: float = 0.08) -> None:
    ...
    """
    from device.adb import get_display_size, tap_raw
...
def paste_text(text: str, *, clear: bool = False, digits: bool = False) -> None:
    ...
    """
    from device.adb import type_digits_raw, type_text_raw
